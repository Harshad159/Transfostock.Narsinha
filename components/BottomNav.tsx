...
const navItems = [
  ...
  { path: '/inward', label: 'Inward', icon: PlusCircleIcon },
  { path: '/outward', label: 'Outward', icon: MinusCircleIcon },
  { path: '/overview', label: 'Stock', icon: ArchiveBoxIcon },
...
const BottomNav: React.FC = () => {
  ...
  const inactiveLink = 'text-text-secondary hover:text-primary';

  return (
    <nav className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto bg-surface border-t border-gray-200 shadow-md z-10">
      <div className="flex justify-around py-2">
        {navItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            className={({ isActive }) => 
                `flex flex-col items-center justify-center w-full text-xs transition-colors duration-200 ${isActive ? activeLink : inactiveLink}`
            }
          >
